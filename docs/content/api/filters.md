...
the menu beside the page title. Source: `services/filters/filters.jac`.
{ .fl-lede }

## Stored filters

A filter dict, on the account or on a saved set, keeps only the keys the page
knows, and only those set to something other than their default. The board's
keys and defaults are one glob, shared by the client and the server's
validation:
...
An empty `set_id` creates a set on `page` (only `board` for now); otherwise the
