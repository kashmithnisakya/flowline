...
INST = 777
STEPS = ["Unassigned issues", "Validated", "Picked", "Design decision",
         "Issues with PRs", "PR merged", "Done"]
...
def main() -> int:
    ...
    check("  Validated needs a due date, Issues with PRs a linked PR",
          by["Validated"]["needs_due_date"] and by["Issues with PRs"]["needs_pr"]
          and not by["Picked"]["needs_due_date"], (by["Validated"], by["Issues with PRs"]))
    ...
    check("  label, merge and review triggers on the right arrows",
          trig("Unassigned issues", "Validated") == ("label", "validated")
          and trig("Picked", "Issues with PRs") == ("label", "ready-to-review")
          and trig("Issues with PRs", "Picked") == ("changes_requested", "")
          and trig("Issues with PRs", "PR merged") == ("pr_merged", "")
          and trig("PR merged", "Done") == ("", ""), [trig("Unassigned issues", "Validated")])
    ...
    check("  they land on Unassigned issues", t10.get("step_id") == by["Unassigned issues"]["id"], t10.get("step_id"))
    check("  a validated label with no due date stays, and the log says why",
          t11.get("step_id") == by["Unassigned issues"]["id"]
          and any(r.get("activity") == "Stayed on Unassigned issues · Validated needs a due date"
                  for r in wg.log_rows()), t11.get("step_id"))
    moved = walker("MoveTask", {"task_id": t10["id"], "step_id": by["Validated"]["id"]})
    check("MoveTask into Validated without a due date is refused",
          moved.get("refused") == "Validated needs a due date" and moved.get("step_id") == by["Unassigned issues"]["id"], moved)
    saved = update(t10, step_id=by["Validated"]["id"], due_date="2026-10-01")
    check("  a save that brings the due date moves it", saved.get("step_id") == by["Validated"]["id"] and not saved.get("refused"), saved)
    check("  and the log names the step", any(r.get("activity") == "Moved to Validated" for r in wg.log_rows()))
    made = walker("CreateTask", {"title": "Straight to review", "project_id": pid, "step_id": by["Issues with PRs"]["id"]})
    check("CreateTask on a step that needs a PR creates nothing", not made.get("id"), made)
    moved = walker("MoveTask", {"task_id": t10["id"], "step_id": by["Issues with PRs"]["id"]})
    check("MoveTask into Issues with PRs without a PR is refused", moved.get("refused") == "Issues with PRs needs a linked PR", moved)
    ...
    check("a validated label on the issue moves it to Validated", task(11).get("step_id") == by["Validated"]["id"], task(11).get("step_id"))
    walker("MoveTask", {"task_id": t11["id"], "step_id": by["Picked"]["id"]})
    ...
    check("a ready-to-review label on the PR moves it to Issues with PRs",
          task(11).get("step_id") == by["Issues with PRs"]["id"], task(11).get("step_id"))
    ...
    check("a review requesting changes sends it back to Picked", task(11).get("step_id") == by["Picked"]["id"], task(11).get("step_id"))
    push("pull_request", envelope("labeled", pull_request=pull(20, iso(6), labels=["Ready-To-Review"])))
    check("  a label it already had does not move it", task(11).get("step_id") == by["Picked"]["id"])
    ...
    check("  labelled again, it goes back to review", task(11).get("step_id") == by["Issues with PRs"]["id"])
    push("pull_request", envelope("closed", pull_request=pull(20, iso(9), labels=["ready-to-review"], merged_at=iso(9))))
    t = task(11)
    check("the merge lands on PR merged, not Done, with auto-done on",
          t.get("step_id") == by["PR merged"]["id"] and t.get("status") == "Review" and t.get("pr_state") == "merged",
    ...
    check("  every move is logged with its reason", all(any(r.get("activity") == a for r in wg.log_rows()) for a in (
        "Moved to Validated · labelled validated", f"Linked PR {REPO} #20",
        "Moved to Issues with PRs · labelled Ready-To-Review", "Moved to Picked · changes requested",
        "Moved to PR merged · PR merged")), [r.get("activity") for r in wg.log_rows()][:12])
    moved = walker("MoveTask", {"task_id": t["id"], "step_id": by["Done"]["id"]})
    check("oversight marks it Done by hand", moved.get("status") == "Done", moved)

    # ------------------------------------------------------ editing triggers
    r = walker("LabelTransition", {"from_id": by["Picked"]["id"], "to_id": by["Issues with PRs"]["id"],
    ...
    r = walker("SetStepRules", {"step_id": by["Validated"]["id"], "needs_due_date": False, "needs_pr": True})
